...
public class MainActivity extends AppCompatActivity {
    ...
    ArrayList<com.alhusseiny.till_man.Message> smsMessageslist = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextChange(String newText) {
                arrayAdapter.filter(newText);//.filter(newText);
                return false;
            }
        });
        ...
        do {
            ...
            if (origin.equalsIgnoreCase("MPESA")) {
                ...
                if (body.contains("received")) {
                    ...
                    String name = body.substring(body.indexOf("from"), body.indexOf(" 07")).replace("from", "").trim();
                    String date = body.substring(body.indexOf(" on "), body.indexOf(" at ")).replace("on", "").trim();
                    String amount = body.substring(body.indexOf("ed Ksh"), body.indexOf(" from ")).replace("ed", "").trim();
                    ;
